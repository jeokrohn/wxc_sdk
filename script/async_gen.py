...
def transform_classes_to_async(sources: Iterable[str]) -> Generator[str, None, None]:
    def transform_method(*, class_name: str, method_match: re.Match) -> str:
        ...
        if RE_GENERATOR.search(source):
            ...
            # rename generator method name to _gen
            gen_source, subs = RE_METHOD_DEF.subn(f'def {method_name}_gen(', gen_source)
            if not subs:
            ...
            # now create an async method
            source, subs = RE_METHOD_DEF.subn(f'async def {method_name}(', source)
            if not subs:
