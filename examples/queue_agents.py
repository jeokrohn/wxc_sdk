...
def get_access_token() -> Optional[Tokens]:
    ...
    integration = Integration(client_id=app_id, client_secret=app_secret, scopes=[], redirect_url=None)
...
async def process_one_queue(
    *, api: AsWebexSimpleApi, queue: CallQueue, agents: list[Person], action: str, dry_run: bool = True
):
    """
    ...
    if action == 'add':
        ...
        agent_str = ', '.join(
            sorted(
                f'{person.display_name}({person.emails[0]})' for person in agents if person.person_id in agents_to_add
            )
        )
        ...
        agent_str = ', '.join(
            sorted(
                f'{person.display_name}({person.emails[0]})'
                for person in agents
                if person.person_id in agents_to_remove
            )
        )
...
async def validate_queues(api: AsWebexSimpleApi, queues: Iterable[str]) -> list[CallQueue]:
    ...
    existing_queues = {f'{queue.location_name}:{queue.name}': queue for queue in await api.telephony.callqueue.list()}
    validated_queues = []
    for queue in queues:
...
def main():
    ...
    action_group.add_argument(
        '--add', action='store_const', const='add', dest='action', help='Add agent(s) to specified queues'
    )
    action_group.add_argument(
        '--remove', action='store_const', const='remove', dest='action', help='Remove agent(s) from specified queues'
    )
    ...
    # fry run option
    parser.add_argument('--dry-run', action='store_true', help='Simulate the operation without making actual changes')
    ...
    # Get access token
    token = args.token
    load_dotenv(os.path.join(os.getcwd(), f'{os.path.splitext(os.path.basename(__file__))[0]}.env'))
        ...
        async with AsWebexSimpleApi(tokens=token) as api:
            with har_writer():
                ...
                validated_queues, validated_agents = await asyncio.gather(
                    validate_queues(api, queues), validate_users(api, agents)
                )
                ...
                await asyncio.gather(
                    *[
                        process_one_queue(
                            api=api, queue=queue, agents=validated_agents, action=args.action, dry_run=args.dry_run
                        )
