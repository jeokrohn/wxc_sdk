...
class DeveloperWebexCom:
    ...
    def param_parser(self, divs: Iterable[Tag], level: int = 0) -> Generator[Parameter, None, None]:
        """
        Parse parameters from divs and
        :param divs:
        :return:
        """
...
def main():
    ...
    with DeveloperWebexCom() as site:
        ...
        try:
            # get method details
            for doc in docs:
                if doc.menu_text not in TARGET_MENUS:
                    logging.info(f'skipping "{doc.menu_text}"')
    ...
    if args.yml_path:
        with open(args.yml_path, mode='w') as f:
            safe_dump(doc_details.dict(), f)
    else:
        safe_dump(doc_details.dict())
