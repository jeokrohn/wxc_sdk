...
__all__ = ['BulkManageSCIM2UsersAndGroupsApi', 'BulkUser', 'BulkUserOperationsItem', 'BulkUserOperationsItemMethod']


class BulkUserOperationsItemMethod(str, Enum):
    post = 'POST'
    patch = 'PATCH'
    delete = 'DELETE'


class BulkUserOperationsItem(ApiModel):
    #: The HTTP method of the current operation.
    method: Optional[BulkUserOperationsItemMethod] = None
...
class BulkUser(ApiModel):
    ...
    operations: Optional[list[BulkUserOperationsItem]] = None
...
class BulkManageSCIM2UsersAndGroupsApi(ApiChild, base='identity/scim'):
    ...
    def user_bulk_api(self, org_id: str, schemas: list[str], fail_on_errors: int,
                      operations: list[BulkUserOperationsItem]) -> BulkUser:
        """
        User bulk API
        ...
        **Usage**:
        ...
        :type operations: list[BulkUserOperationsItem]
        :rtype: :class:`BulkUser`
        """
        body: dict[str, Any] = dict()
        body['schemas'] = schemas
        body['failOnErrors'] = fail_on_errors
        body['operations'] = TypeAdapter(list[BulkUserOperationsItem]).dump_python(operations, mode='json', by_alias=True, exclude_none=True)
