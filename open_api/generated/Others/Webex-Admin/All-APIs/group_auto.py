...
class PatchMemberWithOperation(ApiModel):
    #: Person ID.
    id: Optional[str] = None
    #: Operation type. Either `add` or `delete`. The default operation is `add` if no operation is specified.
...
class GroupsApi(ApiChild, base='groups'):
    ...
    def delete_a_group(self, group_id: str) -> None:
        """
        Delete a Group
    ...
    def update_a_group(self, group_id: str, display_name: str = None, description: str = None,
                       members: list[PatchMemberWithOperation] = None) -> GroupResponse:
        """
        Update a Group
