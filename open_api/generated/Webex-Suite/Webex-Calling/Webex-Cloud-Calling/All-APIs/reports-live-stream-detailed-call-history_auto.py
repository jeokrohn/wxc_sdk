...
__all__ = ['CDR', 'ReportsLiveStreamDetailedCallHistoryApi']
...
class CDR(ApiModel):
    ...
    #: - RouteListCalls: When off-net route list call is blocked due to exceeding the Route List Calls license overage
...
class ReportsLiveStreamDetailedCallHistoryApi(ApiChild, base='cdr_stream'):
    """
    Reports: Live Stream Detailed Call History
    ...
    **analytics-calling-gov.webexapis.com** for Government). These endpoints are
    ...
    We recommend using the CDR stream endpoint as the primary API for customers who need to continuously consume CDRs
    for custom reporting, analytics applications, or integration into internal data stores.
    
    The CDR Stream endpoint will allow customers to call the API as soon as the call ends, typically 1 minute before
    the current time. It automatically backfills any late records as soon as they are processed in the Webex Calling
    cloud. These records are then delivered in subsequent API calls, ensuring reliable CDR delivery.
    ...
    reaches the Webex Calling cloud at 9:46 AM, the record becomes available from 9:47 AM and remains retrievable
    until 9:46 PM the same day. Additionally, only 2 hours’ worth of records can be retrieved per request (i.e., 2
    hours between the selected start and end times in a single API call).
    ...
    Details on the returned fields and their possible values are available at
    <https://help.webex.com/en-us/article/nmug598/Reports-for-Your-Cloud-Collaboration-Portfolio>. Select the **Report
    templates** tab, then under **Webex Calling reports**, see **Calling Detailed Call History Report**.
    
    By default, requests to analytics-calling.webexapis.com are routed to the closest region's servers. If those
    servers host the organization’s data, the data is returned. Otherwise, an HTTP 451 error code is returned. In such
    cases, the response body contains endpoint information indicating where the organization’s data can be retrieved.
    """
    ...
    def get_live_stream_detailed_call_history(self, start_time: Union[str, datetime], end_time: Union[str, datetime],
        ...
        `endTime` parameters specify the time window during which Detailed Call History data was inserted into the
        Webex Calling cloud. The API will return all reports whose insertion time into the Webex Calling cloud falls
        ...
        :param start_time: The start date-time of the first record you wish to collect in UTC time. It would be the
            earliest time at which the data was inserted into the Webex Calling cloud for the records you wish to
            collect. Format must be as `YYYY-MM-DDTHH:MM:SS.mmmZ`. `startTime` can't be older than 12 hours from your
            current UTC time. The window period between `startTime` and `endTime` must not exceed 2 hours in a single
            API request.
        :type start_time: Union[str, datetime]
        :param end_time: The end date-time of the last record you wish to collect in UTC time. It would be the latest
            time at which the data was inserted into the Webex Calling cloud for the records you wish to collect.
            Format must be as `YYYY-MM-DDTHH:MM:SS.mmmZ`. `endTime` must be 1 minute ago from your current UTC time
            and can’t be older than 12 hours. `endTime` must be greater than `startTime`. The window period between
            `startTime` and `endTime` must not exceed 2 hours in a single API request.
