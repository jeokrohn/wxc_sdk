...
class LicensesApi(ApiChild, base='licenses'):
    ...
    def assign_licenses_to_users(self, email: str = None, person_id: str = None, licenses: list[LicenseRequest] = None,
        ...
        if licenses is not None:
            body['licenses'] = loads(TypeAdapter(list[LicenseRequest]).dump_json(licenses, by_alias=True, exclude_none=True))
        if site_urls is not None:
            body['siteUrls'] = loads(TypeAdapter(list[SiteUrlsRequest]).dump_json(site_urls, by_alias=True, exclude_none=True))
        url = self.ep('users')
