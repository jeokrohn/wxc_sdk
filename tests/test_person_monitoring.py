...
def print_monitoring(*, user: Person, monitoring: Monitoring):
    ...
    for me in monitoring.monitored_elements or []:
        if me.member:
            ...
            print(f'    {me.cpe.name} (CPE) in {me.cpe.location}')
        elif me.speed_dial:
            print(f'    {me.speed_dial.display_name} (SD): {me.speed_dial}')
            ...
        else:
            print(f'    unknown monitored element: {me}')
    print('Data for update:')
    print(json.dumps(monitoring.update(), indent=2))
...
class TestRead(TestCaseWithUsers):
    def test_001_read_all(self):
        ...
        """
        mo = self.api.person_settings.monitoring

        with ThreadPoolExecutor() as pool:
            monitoring_settings = list(pool.map(lambda user: mo.read(entity_id=user.person_id), self.users))
        print(f'Got monitoring details for {len(self.users)} users')
        with_elements = [
            (user, ms) for user, ms in zip(self.users, monitoring_settings, strict=True) if ms.monitored_elements
        ]
        for user, ms in with_elements:
            ...
            if ms.monitored_elements:
                # try to update
                mo.configure(entity_id=user.person_id, settings=ms)
                ms_after = mo.read(entity_id=user.person_id)
                self.assertEqual(ms, ms_after)

    def test_002_get_available_members_for_monitoring_all(self):
        """
        Get available members for monitoring for all users
        """
        mo = self.api.person_settings.monitoring
        with ThreadPoolExecutor() as pool:
            available_members = list(
                pool.map(
                    lambda user: list(mo.get_available_members_for_monitoring(entity_id=user.person_id)), self.users
                )
            )
        print(f'Got available members for monitoring for {len(self.users)} users')
        for user, members in zip(self.users, available_members, strict=True):
            members: list[MonitoringMember]
            print(f'**** user "{user.display_name}({user.emails[0]})"')
            print(
                json.dumps(
                    TypeAdapter(list[MonitoringMember]).dump_python(
                        members, mode='json', by_alias=True, exclude_none=True
                    ),
                    indent=2,
                )
            )
    ...
    def test_003_get_available_speed_dials_for_monitoring_all(self):
        """
        Get available speed dials for monitoring for all users
        """
        mo = self.api.person_settings.monitoring
        with ThreadPoolExecutor() as pool:
            available_members = list(
                pool.map(
                    lambda user: list(mo.get_available_speed_dials_for_monitoring(entity_id=user.person_id)), self.users
                )
            )
        print(f'Got available speed dials for monitoring for {len(self.users)} users')
        for user, members in zip(self.users, available_members, strict=True):
            members: list[MonitoringMember]
            print(f'**** user "{user.display_name}({user.emails[0]})"')
            print(
...
class TempCPE:
    ...
    async def cleanup(self):
        async with AsWebexSimpleApi(tokens=self.api.access_token) as api:
            tasks = [
                api.telephony.callpark_extension.delete(location_id=self.location_id, cpe_id=cpe_id)
    ...
    def generate_cpes(self, cpes_present: list[CallParkExtension], cpe_count: int = 1) -> list[str]:
        ...
        async def get_cpes() -> list[str]:
            async with AsWebexSimpleApi(tokens=self.api.access_token) as api:
                ...
                available_ci_cpe_ids = [cpe.cpe_id for cpe in existing_cpes if cpe.cpe_id not in ids_cpes_present]
...
class TestUpdate(TestCaseWithUsers):
    ...
    def test_003_add_user_by_id(self):
        ...
        with self.target_user() as user:
            ...
            user_candidates = [
                uc for uc in self.users if uc.person_id not in present_ids and uc.person_id != user.person_id
            ]
    ...
    def test_004_verify_user_id_format(self):
        ...
        with self.target_user() as user:
            ...
            before = mon.read(entity_id=user.person_id)
            present_ids = [m.member_id for m in before.monitored_members]
            user_candidates = [user for user in self.users if user.person_id not in present_ids]
            to_add = random.sample(user_candidates, 3)
            print(f'Trying to add monitoring for: {", ".join(u.display_name for u in to_add)}')
        ...
        decoded_user_ids = list(
            map(lambda member: base64.b64decode(member.member_id + '==').decode(), after.monitored_members)
        )
        for member, decoded in zip(after.monitored_members, decoded_user_ids, strict=True):
