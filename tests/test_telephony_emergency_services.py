...
class TestEmergencyServices(TestWithLocations):
    def test_get_org_settings(self):
        ...
        """
        settings = self.api.telephony.emergency_services.get_notification()
    ...
    async def get_all_location_settings(
    ...
    ) -> list[Union[Exception, LocationEmergencyCallNotification]]:
        """
        ...
        settings_list = await asyncio.gather(
            *[api.read_emergency_call_notification(location_id=loc.location_id) for loc in self.locations],
            return_exceptions=return_exceptions,
        )
        return settings_list  # type: ignore[return-value]
    ...
    async def test_read_all_locations(self):
        ...
        for location, setting in zip(self.locations, settings_list):
            ...
            print(
                '\n'.join(
                    f'  {line}'
                    for line in json.dumps(setting.model_dump(mode='json', by_alias=True), indent=2).splitlines()
                )
            )
        if err:
            raise err
    ...
    async def test_update_location_enable(self):
        ...
        candidates = [
            (loc, setting)
            for loc, setting in zip(self.locations, settings_list)
        ...
        setting: LocationEmergencyCallNotification
        update = LocationEmergencyCallNotification(
            emergency_call_notification_enabled=True, email_address='jkrohn@cisco.com'
        )
        ...
        try:
            ...
            self.assertEqual(update.emergency_call_notification_enabled, after.emergency_call_notification_enabled)
            self.assertEqual(update.email_address, after.email_address)
        finally:
...
class TestUserECBN(TestCaseWithUsers):
    ...
    async def test_get_ecbn_all_users(self):
        ...
        settings_list = await asyncio.gather(
            *[api.read(entity_id=user.person_id) for user in self.users], return_exceptions=True
        )
        err = None
    ...
    async def test_get_dependencies_all_users(self):
        ...
        dependencies_list = await asyncio.gather(
            *[api.dependencies(entity_id=user.person_id) for user in self.users], return_exceptions=True
        )
        err = None
        for user, dependencies in zip(self.users, dependencies_list):
            ...
            print(
                '\n'.join(
                    f'  {line}'
                    for line in json.dumps(dependencies.model_dump(mode='json', by_alias=True), indent=2).splitlines()
                )
            )
        if err:
            raise err
    ...
    def test_update_anita_hunt(self):
        ...
        selected = (
            SelectedECBN.direct_line
            if ecbn_settings.selected != SelectedECBN.direct_line
            else (SelectedECBN.location_ecbn)
        )
    ...
    def test_update_user_location_member(self):
        ...
        numbers = [
            number
            for number in self.api.telephony.phone_numbers(number_type=NumberType.number)
            if number.owner and number.owner.owner_type == OwnerType.people
        ]
        user_ids_by_location_id: dict[str, set[str]] = reduce(
            lambda r, e: r[e.location.id].add(e.owner.owner_id) or r, numbers, defaultdict(set)
        )
        user_id_list = list(
            chain.from_iterable(user_ids for user_ids in user_ids_by_location_id.values() if len(user_ids) > 1)
        )
        if not user_id_list:
        ...
        numbers = list(
            self.api.telephony.phone_numbers(
                number_type=NumberType.number, owner_type=OwnerType.people, location_id=target.location_id
            )
        )
        ...
        try:
            self.assertEqual(after.selected, SelectedECBN.location_member_number)
            self.assertEqual(target_number.owner.owner_id, after.location_member_info.member_id)
            self.assertEqual(dependencies_before.dependent_member_count + 1, dependencies_after.dependent_member_count)
        finally:
            # reset the ECBN
            ecbn_api.configure(
                entity_id=target.person_id,
                ...
                location_member_id=(
                    before.selected == SelectedECBN.location_member_number
                    and before.location_member_info.member_id
                    or None
                ),
            )
