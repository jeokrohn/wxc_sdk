...
def filtered(targets, name_getter=None, alternate_matches: Union[Pattern, str] = None, only_alternate: bool = False):
...
def delete_inactive_users(pool: ThreadPoolExecutor, api: WebexSimpleApi):
    def is_calling_user(user: Person) -> bool:
        return any((lic := licenses_by_id.get(lic_id)) and lic.webex_calling for lic_id in user.licenses)
    ...
    if not DRY_RUN:
        list(pool.map(lambda u: api.people.delete_person(person_id=u.person_id), to_delete))
    return
...
def cleanup_devices(pool: ThreadPoolExecutor, api: WebexSimpleApi):
    ...
    mpp_devices = [
        device
        for device in api.devices.list(product_type='phone')
        if (
            device.mac
    ...
    if False:
        ...
        user_devices = chain.from_iterable(
            pool.map(lambda user: api.person_settings.devices(person_id=user.person_id).devices, users)
        )
        workspaces = list(api.workspaces.list(calling=CallingType.webex))
        workspace_devices = chain.from_iterable(
            pool.map(lambda ws: api.workspace_settings.devices.list(workspace_id=ws.workspace_id), workspaces)
        )
        user_and_workspace_devices = [
            d
            for d in chain(user_devices, workspace_devices)
            if (
                d.device_id not in mpp_ids
                and (
                    d.mac and d.mac.startswith('DEADDEAD') or d.activation_state and d.activation_state == 'activating'
                )
            )
        ]
    else:
...
async def main():
    ...
    with HarWriter(log_name.replace('.log', '.har'), api) as har_writer:
        ...
        with ThreadPoolExecutor() as pool:
            ...
            if False:
                ...
                if dustins:
                    ...
                    details = list(
                        pool.map(
                            lambda cq: api.telephony.callqueue.details(location_id=cq.location_id, queue_id=cq.id),
                            call_queues,
                        )
                    )
                    remove_from = [
                        cq
                        for cq in details
                        if next((agent for agent in cq.agents if agent.agent_id == dustin.person_id), None) is not None
                    ]
                    ...
                    if remove_from:
                        list(
                            pool.map(
                                lambda cq: api.telephony.callqueue.update(
                                    location_id=cq.location_id, queue_id=cq.id, update=queue_wo_dustin(cq)
                                ),
                                remove_from,
                            )
                        )
            ...
            if not DRY_RUN:
                list(
                    pool.map(
                        lambda aa: ata.delete_auto_attendant(
                            location_id=aa.location_id, auto_attendant_id=aa.auto_attendant_id
                        ),
                        aa_list,
                    )
                )
            ...
            cp_list = list(
                filtered(
                    chain.from_iterable(pool.map(lambda l: atc.list(location_id=l.location_id), locations)),
                    alternate_matches=r'CPG\d',
                )
            )
            ...
            if not DRY_RUN:
                list(
                    pool.map(
                        lambda cp: atc.delete_callpark(location_id=cp.location_id, callpark_id=cp.callpark_id), cp_list
                    )
                )
            ...
            cpu_list = list(
                filtered(chain.from_iterable(pool.map(lambda l: atp.list(location_id=l.location_id), locations)))
            )
            print(f'deleting {len(cpu_list)} call pickups: {", ".join(cp.name for cp in cpu_list)}')
            if not DRY_RUN:
                list(
                    pool.map(lambda cp: atp.delete_pickup(location_id=cp.location_id, pickup_id=cp.pickup_id), cpu_list)
                )
            ...
            if not DRY_RUN:
                list(pool.map(lambda cq: atq.delete_queue(location_id=cq.location_id, queue_id=cq.id), cq_list))

            # hunt groups
            ath = api.telephony.huntgroup
            hg_list = list(
                filtered(chain.from_iterable(pool.map(lambda l: ath.list(location_id=l.location_id), locations)))
            )
            print(f'deleting {len(hg_list)} hunt groups: {", ".join(hg.name for hg in hg_list)}')
            if not DRY_RUN:
                list(pool.map(lambda hg: ath.delete_huntgroup(location_id=hg.location_id, huntgroup_id=hg.id), hg_list))
            ...
            if not DRY_RUN:
                list(
                    pool.map(lambda pg: atpg.delete_paging(location_id=pg.location_id, paging_id=pg.paging_id), pg_list)
                )
            ...
            schedule_list = list(
                filtered(
                    chain.from_iterable(pool.map(lambda l: ats.list(obj_id=l.location_id), locations)),
                    alternate_matches=r'\w+ \d{2}',
                )
            )
            ...
            if not DRY_RUN:
                list(
                    pool.map(
                        lambda schedule: ats.delete_schedule(
            ...
            users = [user for user in api.people.list(calling_data=True) if user.location_id]

            schedule_lists = list(pool.map(lambda user: list(aps.list(obj_id=user.person_id)), users))
            ...
            print(
                f'deleting {len(users_and_schedules)} user schedules:'
            ...
            # groups
            groups = list(filtered(api.groups.list(), name_getter=lambda g: g.display_name))
            groups.sort(key=lambda g: g.display_name)
            print(f'Deleting {len(groups)} groups')
            if not DRY_RUN:
                list(pool.map(lambda g: api.groups.delete_group(group_id=g.group_id), groups))

            # workspaces
            workspaces = list(filtered(api.workspaces.list(), name_getter=lambda w: w.display_name))
            print(f'Deleting {len(workspaces)} workspaces')
            if not DRY_RUN:
                list(pool.map(lambda ws: api.workspaces.delete_workspace(workspace_id=ws.workspace_id), workspaces))
            ...
            if not DRY_RUN:
                list(
                    pool.map(
                        lambda g: api.telephony.voicemail_groups.delete(
                            location_id=g.location_id, voicemail_group_id=g.group_id
                        ),
                        groups,
                    )
                )
            # route lists
            route_lists = list(
                filtered(api.telephony.prem_pstn.route_list.list(), alternate_matches=r'.+ \d{2}$', only_alternate=True)
            )
            if not DRY_RUN:
                list(pool.map(lambda rl: api.telephony.prem_pstn.route_list.delete_route_list(rl.rl_id), route_lists))

            # dial plans
            dial_plans = list(
                filtered(api.telephony.prem_pstn.dial_plan.list(), alternate_matches=r'.+ \d{2}$', only_alternate=True)
            )
            if not DRY_RUN:
                list(
                    pool.map(
            ...
            route_groups = list(
                filtered(
                    api.telephony.prem_pstn.route_group.list(), alternate_matches=r'.+ \d{2}$', only_alternate=True
                )
            )
            if not DRY_RUN:
                list(
                    pool.map(lambda rg: api.telephony.prem_pstn.route_group.delete_route_group(rg.rg_id), route_groups)
                )

            # trunks
            trunks = list(
                filtered(api.telephony.prem_pstn.trunk.list(), alternate_matches=r'.+ \d{2}$', only_alternate=True)
            )
            if not DRY_RUN:
                list(pool.map(lambda t: api.telephony.prem_pstn.trunk.delete_trunk(trunk_id=t.trunk_id), trunks))
            ...
            if numbers_by_location:
                ...
                print(
                    '\n'.join(
                        f'  {l_name:{loc_len}} ({len(numbers)} numbers): '
            ...
            if not DRY_RUN:
                ...
                list(
                    pool.map(
                        lambda v: api.telephony.location.number.remove(location_id=v[0], phone_numbers=v[1]), batches
                    )
                )
            ...
            if not DRY_RUN:
                list(
                    pool.map(
                        lambda cpe: api.telephony.callpark_extension.delete(
                            location_id=cpe.location_id, cpe_id=cpe.cpe_id
                        ),
                        cpe_list,
                    )
                )
            ...
            # teams
            teams = list(filtered(api.teams.list(), alternate_matches=r'Team Test \d{3}'))
            ...
            scim_users = [u for u in scim_api.scim.users.search_all(org_id=org_id) if u.external_id]
            ...
            # SCIM groups
            scim_groups = [g for g in scim_api.scim.groups.search_all(org_id=org_id) if g.external_id]
        ...
        async with AsWebexSimpleApi(tokens=tokens) as as_api:
            ...
            if False:
                locations = [
                    loc for loc in api.workspace_locations.list() if re.search(r'\b(\w{2})\b (\d+) USA', loc.address)
                ]
                print(f'Deleting {len(locations)} locations')
                if not DRY_RUN:
                    await asyncio.gather(
                        *[as_api.workspace_locations.delete(location_id=loc.id) for loc in locations],
                        return_exceptions=True,
                    )
            ...
            async def remove_ocp_patterns_user(user: Person):
                try:
                    ...
                    patterns_to_delete: list[DigitPattern] = list(filtered(patterns, alternate_matches=r'^ocp\-\+'))
                    if not patterns_to_delete:
                        return
                    print(f'Removing {len(patterns_to_delete)} patterns for user {user.display_name}')
                    await asyncio.gather(
                        *[dpapi.delete(user.person_id, digit_pattern_id=pattern.id) for pattern in patterns_to_delete],
                        return_exceptions=True,
                    )
                except Exception as e:
                    print(f'Error removing patterns for user {user.display_name}: {e}')

            await asyncio.gather(*[remove_ocp_patterns_user(user) for user in users], return_exceptions=True)
            ...
            async def remove_ocp_patterns_location(location: TelephonyLocation):
                try:
                    ...
                    await asyncio.gather(
                        *[
                            dpapi.delete(location.location_id, digit_pattern_id=pattern.id)
