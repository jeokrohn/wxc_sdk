...
class TestPeople(TestCaseWithLog):
    ...
    def test_005_create_user(self):
        ...
        email_re = re.compile(rf'{base_email[0]}\+.+@{base_email[1]}')

        # users with email matching that re
        test_users = [user for user in self.api.people.list() if any(email_re.match(email) for email in user.emails)]
    ...
    def test_006_list_by_id_and_check_for_user_name(self):
        ...
        self.assertTrue(
            all('userName' not in item for item in list_requests[0].response_body.get('items')),
            "userName returned in 'regular' list",
        )
        # second request has 'userName'
        self.assertTrue(
            all('userName' in item for item in list_requests[1].response_body.get('items')),
    ...
    async def test_007_get_details_by_uuid(self):
        ...
        details_list = await asyncio.gather(
            *[self.async_api.people.details(person_id=webex_id_to_uuid(user.person_id)) for user in users]
        )
    ...
    async def test_001_show_numbers(self):
        ...
        for person, person_numbers in zip(user_details, user_numbers):
            ...
            if len(person.phone_numbers) != len(person_numbers.phone_numbers):
                person_error = True
                print(
                    f' different # of phone numbers via person API and person_settings '
                    f'({len(person.phone_numbers)} != {len(person_numbers.phone_numbers)})'
                )
                extensions = [pn.extension for pn in person_numbers.phone_numbers if pn.extension]
                direct_numbers = [pn.direct_number for pn in person_numbers.phone_numbers if pn.direct_number]
                person_phone_numbers = [pn.value for pn in person.phone_numbers]
                extensions_not_in_person = [ext for ext in extensions if ext not in person_phone_numbers]
                if extensions_not_in_person:
                    print(f' extensions not in person data: {", ".join(extensions_not_in_person)}')
                direct_numbers_not_in_person = [dn for dn in direct_numbers if dn not in person_phone_numbers]
                if direct_numbers_not_in_person:
                    print(f' direct numbers no in person data: {", ".join(direct_numbers_not_in_person)}')
                person_phone_number_not_in_person_settings = [
                    p for p in person_phone_numbers if p not in extensions and p not in direct_numbers
                ]
                if person_phone_number_not_in_person_settings:
                    print(
                        f'  person phone numbers not an extension or direct number: '
                        f'{", ".join(person_phone_number_not_in_person_settings)}',
                        end='',
                    )

                    # some numbers might actually end with an extension: these probably are ESN?
                    person_phone_numbers_esn = [
                        p
                        for p in person_phone_number_not_in_person_settings
...
class TestCallingUser(TestWithLocations):
    def test_create_random_calling_user(self):
        ...
        new_user = create_random_calling_user(api=self.api, location_id=target_location.location_id, with_tn=True)
        print(new_user)
...
class TestCallingLocations(TestWithLocations):
    ...
    async def test_list_people_by_location(self):
        async def list_people_in_location(location: TelephonyLocation):
            ...
            print(f'Location: {location.name} ({location.location_id}) - Found {len(users)} users with calling data')
            if not users:
                return
            # verify that location is set for all users
            self.assertTrue(
                all(user.location_id == location.location_id for user in users),
                f'Not all users in location {location.name} ({location.location_id}) have the same location_id',
            )

            # get details for each user to verify location info
            details = await asyncio.gather(
                *[self.async_api.people.details(person_id=user.person_id, calling_data=True) for user in users],
            ...
            for user, list_user in zip(details, users):
                ...
                if user.location_id != location.location_id:
                    user_location = next(
                        (loc for loc in self.telephony_locations if loc.location_id == user.location_id), None
                    )
                    if user_location is None:
                        print(
                            f'  User {user.display_name} has location {user.location_id} '
