...
class TestMessages(TestCaseWithUsersAndSpaces):
    ...
    def test_003_create_direct_message_local_file(self):
        ...
        # pick a random user
        target_user = choice(self.users)
        new_message = api.create(
            to_person_id=target_user.person_id, text=f'Random message {uuid.uuid4()} with attachment', files=[__file__]
        )
        # now a space has to exist with that user
        self.assertEqual(RoomType.direct, new_message.room_type)
        ...
        attachments = [
            {
                ...
                'content': {
                    'type': 'AdaptiveCard',
                    'version': '1.0',
                    'body': [{'type': 'TextBlock', 'text': 'Sample Adaptive Card', 'size': 'large'}],
...
class TestMembership(TestCaseWithUsersAndSpaces):
    def test_001_list_memberships(self):
        ...
        for space, memberships in zip(spaces, memberships_list, strict=True):
            print(space.title)
...
class TestRooms(TestCaseWithLog):
    ...
    def test_002_create(self):
        rooms = list(self.api.rooms.list())
        new_names = (name for i in range(1000) if (name := f'Test Space {i:03}') not in set(r.title for r in rooms))
    ...
    def test_004_meeting_details(self):
        ...
        rooms = [room for room in self.api.rooms.list() if not room.is_announcement_only]
        with ThreadPoolExecutor() as pool:
    ...
    def target_space(self):
        with self.no_log():
            rooms = [r for r in self.api.rooms.list() if TEST_NAME.match(r.title)]
        if not rooms:
...
class TestTeams(TestCaseWithLog):
        ...
        if self.users is None:
            with self.no_log():
                ...
                self.__class__.users = [
                    user async for user in self.async_api.people.list_gen() if user.person_id != me.person_id
                ]
        return self.users

    async def get_teams(self) -> list[Team]:
        return [team async for team in self.async_api.teams.list_gen() if re.match(r'Team Test \d{3}', team.name)]
        ...
        general_space = next(
            iter(
                await asyncio.gather(
                    ...
                    *[
                        self.async_api.team_memberships.create(team_id=team.id, person_id=member.person_id)
    ...
    async def test_001_create(self):
        ...
        users: list[Person]
        teams: list[Team]
        new_names = (name for i in range(1000) if (name := f'Team Test {i:03}') not in set(team.name for team in teams))
        ...
        async def create_team():
            ...
            tasks = []
            user_lists = list(
                chain.from_iterable(combinations(team_members, c_len) for c_len in range(2, len(team_members)))
            )
            shuffle(user_lists)
    ...
    async def test_003_delete_all(self):
        ...
        """
        api = self.async_api
        teams = [team async for team in api.teams.list_gen() if re.match(r'Team Test \d{3}', team.name)]
        await asyncio.gather(*[api.teams.delete(team_id=team.id) for team in teams])
    ...
    async def test_004_teams_with_spaces(self):
        ...
        async def team_with_spaces(name: str, members: list[Person]):
            ...
            await asyncio.gather(
                *[
                    self.async_api.rooms.create(title=f'{team.name}-{i:03}', team_id=team.id)
        ...
        await asyncio.gather(
            *[
                team_with_spaces(name=next(new_names), members=sample(users, self.USERS_IN_TEAM))
