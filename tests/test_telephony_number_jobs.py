...
class TestMoveNumbers(TestWithLocations):
    async def pick_locations(self) -> tuple[LocationAndNumbers, LocationAndNumbers]:
        ...
        """
        locations = [loc for loc in self.locations if loc.address.country == 'US']
        ...
        candidates = [loc for loc in details if loc.connection and numbers_by_location.get(loc.location_id)]
        if len(candidates) < 2:
            self.skipTest('Need at least two locations with numbers to run this test')
        shuffle(candidates)
        return (
            LocationAndNumbers(candidates[0], numbers_by_location[candidates[0].location_id]),
    ...
    def update_pstn(self, location: TelephonyLocation):
        ...
        """
        self.api.telephony.pstn.configure(
            location_id=location.location_id,
            premise_route_id=location.connection.id,
    ...
    async def delete_numbers(self, location_id: str, numbers: list[str]):
        ...
        await asyncio.gather(
            *[
                self.async_api.telephony.location.number.remove(
                    location_id=location_id, phone_numbers=numbers[i : i + 5]
                )
    ...
    async def cleanup_numbers(self, location: TelephonyLocation, loc_ref: str, new_tns: list[str]):
        ...
        numbers = set(
            n.phone_number
            for n in self.api.telephony.phone_numbers(location_id=location.location_id, number_type=NumberType.number)
        )
        to_remove = [n for n in new_tns if n in numbers]
        if to_remove:
            await self.delete_numbers(location_id=location.location_id, numbers=to_remove)
            print(f'removed numbers from {loc_ref} location {location.name}: {", ".join(to_remove)}')
        return

    @asynccontextmanager
    async def move_number_context(
        self, number_count: int = 1, number_state: NumberState = NumberState.active, keep_log: bool = True
    ) -> MoveNumberContext:
        ...
        self.api.telephony.location.number.add(
            location_id=source_location.location.location_id, phone_numbers=new_tns, state=number_state
        )
        try:
            ...
            try:
                yield MoveNumberContext(
                    source_location=source_location, target_location=target_location, numbers=new_tns
                )
            finally:
                with self.no_log(keep_log):
    ...
    def job_completed(self, job: NumberJob) -> bool:
        ...
        completed = job.latest_execution_status == 'COMPLETED' and all(
            jes.exit_code == 'COMPLETED'
            and all(ses.status_message == 'COMPLETED' for ses in jes.step_execution_statuses)
    ...
    async def move_number(self, number_count: int = 1, number_state: NumberState = NumberState.active):
        """
        ...
        async with self.move_number_context(number_count=number_count, number_state=number_state) as context:
            ...
            job = self.api.telephony.jobs.manage_numbers.initiate_job(
                ...
                number_list=[NumberItem(location_id=source_location.location.location_id, numbers=new_tns)],
            )
            ...
            numbers_in_target_location = set(
                tn.phone_number
                for tn in self.api.telephony.phone_numbers(
                    location_id=target_location.location.location_id, number_type=NumberType.number
                )
            )
            new_tns_not_moved = [tn for tn in new_tns if tn not in numbers_in_target_location]
    ...
    @skip('This should now actually work. Update Test...')
    @async_test
    async def test_002_move_numbers(self):
        ...
        """
        # TODO: update test; this should now actually work
