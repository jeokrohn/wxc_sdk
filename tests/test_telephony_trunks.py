...
class TestCreate(TestWithLocations):
    ...
    def test_001_create(self):
        ...
        new_trunk_id = self.api.telephony.prem_pstn.trunk.create(
            name=trunk_name, location_id=location.location_id, password=password, trunk_type=TrunkType.registering
        )
        try:
...
class TestUsage(TestCaseWithLog):
    ...
    def test_001_get_usage_all(self):
        ...
        with ThreadPoolExecutor() as pool:
            details = list(pool.map(lambda t: api.usage(trunk_id=t.trunk_id), trunks))
    ...
    def test_002_get_usage_dial_plan(self):
        ...
        trunks = list(api.list())
        with ThreadPoolExecutor() as pool:
            usage = list(pool.map(lambda t: list(api.usage_dial_plan(trunk_id=t.trunk_id)), trunks))
    ...
    def test_003_get_usage_location_pstn(self):
        ...
        trunks = list(api.list())
        with ThreadPoolExecutor() as pool:
            usage = list(pool.map(lambda t: list(api.usage_location_pstn(trunk_id=t.trunk_id)), trunks))
    ...
    def test_004_get_usage_route_group(self):
        ...
        trunks = list(api.list())
        with ThreadPoolExecutor() as pool:
            usage = list(pool.map(lambda t: list(api.usage_route_group(trunk_id=t.trunk_id)), trunks))
    ...
    def test_004_get_usage_call_to_extension(self):
        ...
        trunks = list(api.list())
        with ThreadPoolExecutor() as pool:
            usage = list(pool.map(lambda t: list(api.usage_call_to_extension(trunk_id=t.trunk_id)), trunks))
...
class TestValidateFQDNandDomain(TestCaseWithLog):
    ...
    async def cert_trunks(self) -> list[TrunkDetail]:
        ...
        if self._cert_trunks is None:
            cert_trunks = [
                t
                for t in await self.async_api.telephony.prem_pstn.trunk.list()
            ...
            if not cert_trunks:
                ...
                self.__class__._cert_trunks = await asyncio.gather(
                    *[self.async_api.telephony.prem_pstn.trunk.details(trunk_id=t.trunk_id) for t in cert_trunks]
                )
        # noinspection PyTypeChecker
    ...
    async def available_fqdn(self) -> AddressTuple:
        ...
        return next(
            p for i in range(1, 100) if (p := AddressTuple(f'cube_{i:02}', self.verified_domain)) not in existing
        )
    ...
    def test_001_invalid_domain(self):
        ...
        self.assertTrue(rest_error.code == 25024 and rest_error.response.status_code == 400, f'{rest_error}')
    ...
    async def test_002_valid_fqdn(self):
        ...
        self.api.telephony.prem_pstn.trunk.validate_fqdn_and_domain(address=fqdn.address, domain=fqdn.domain, port=6001)
    ...
    async def test_004_existing_fqdn_and_port(self):
        ...
        """
        cert_trunks = [t for t in await self.cert_trunks() if t.port]
        if not cert_trunks:
            self.skipTest('Need a certificate based trunk (FQDN and port) to run test')
        target = choice(cert_trunks)
        with self.assertRaises(RestError) as ctx:
            self.api.telephony.prem_pstn.trunk.validate_fqdn_and_domain(
                address=target.address, domain=target.domain, port=target.port
            )
        rest_error: RestError = ctx.exception
        self.assertTrue(rest_error.code == 27571 and rest_error.response.status_code == 409, f'{rest_error}')
    ...
    async def test_005_existing_srv(self):
        ...
        Existing SRV
        """
        cert_trunks = [t for t in await self.cert_trunks() if t.port is None]
        if not cert_trunks:
...
class TestDeleteTestTrunks(TestCaseWithLog):
    def test_001_delete_test_trunks(self):
        trunks = list(self.api.telephony.prem_pstn.trunk.list())
        to_delete = [trunk for trunk in trunks if re.match(r'.+\s\d{2}$', trunk.name)]
        if not to_delete:
            self.skipTest('Nothing to do')
        with ThreadPoolExecutor() as pool:
            list(
                pool.map(
                    lambda trunk: self.api.telephony.prem_pstn.trunk.delete_trunk(trunk_id=trunk.trunk_id), to_delete
                )
            )
